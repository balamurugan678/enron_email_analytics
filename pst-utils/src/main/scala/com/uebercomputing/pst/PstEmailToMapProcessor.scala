    ...
    if (email.hasAttachments()) {
      ...
      for (i <- 0 until attachmentCount) {
        try {
          ...
          case e: Exception => {
            LOGGER.warn(s"Unable to process file attachment ${i} " +
              s"in ${email.getDescriptorNodeId} in ${map(OriginalPstPathKey)} due to ${e}")
          }
        }
      }
  ...
  def addRecipients(email: PSTMessage, map: mutable.Map[String, AnyRef]): Unit = {
    ...
    for (i <- 0 until recipientsCount) {
      try {
        ...
        case e: Exception => LOGGER.warn(s"Unable to get recipient ${i}" +
          s" in ${email.getDescriptorNodeId} in ${map(OriginalPstPathKey)} due to ${e}")
      }
    }
    if (tos.size() > 0) {
